"""
Source: https://github.com/evilwb/APRac2/blob/main/pcsx2_interface/pine.py
...
class Pine:
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, slot: int = 28011):
        if not 0 < slot <= 65536:
    ...
    async def _init_socket(self) -> None:
        if system() == "Windows":
            ...
            socket_family = socket.AF_UNIX # pyright: ignore[reportAttributeAccessIssue]
    ...
    async def read_int8(self, address: int) -> int:
    ...
    async def read_bytes(self, address: int, length: int) -> bytes:
        """Careful! This can be quite slow for large reads"""
        data = b''
        while len(data) < length:
            if length - len(data) >= 8:
                data += (await self._send_request(Pine._create_request(Pine.IPCCommand.READ64, address + len(data), 9)))[-8:]
            elif length - len(data) >= 4:
                data += (await self._send_request(Pine._create_request(Pine.IPCCommand.READ32, address + len(data), 9)))[-4:]
            elif length - len(data) >= 2:
                data += (await self._send_request(Pine._create_request(Pine.IPCCommand.READ16, address + len(data), 9)))[-2:]
            elif length - len(data) >= 1:
                data += (await self._send_request(Pine._create_request(Pine.IPCCommand.READ8, address + len(data), 9)))[-1:]

        return data

    async def write_int8(self, address: int, value: int) -> None:
    ...
    async def write_int32(self, address: int, value: int) -> None:
    ...
    async def write_int64(self, address: int, value: int) -> None:
    ...
    async def write_float(self, address: int, value: float) -> None:
        request = Pine._create_request(Pine.IPCCommand.WRITE32, address, 9 + Pine.DataSize.INT32)
    ...
    async def get_game_id(self) -> str:
